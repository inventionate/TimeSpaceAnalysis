...
fviz_gda_structure <- function(res_gda,
  ...
  var_mod <-
    ...
    separate(rowname, c("var1_mod", "var2_mod"), sep = "_") %>%
    select(var1_mod, var2_mod) %>%
  ...
  var_mod_names <-
    ...
    arrange(var1_mod, desc(var2_mod)) %>%
    .$rowname
  ...
  df_coord1 <- c(
    ref1,
    ref1 + res_crossing$reg$Axis.1$coefficients[-c(1:var_mod$var1_mod), 1]
  )
  ...
  df_coord2 <- c(
    ref2,
    ref2 + res_crossing$reg$Axis.2$coefficients[-c(1:var_mod$var1_mod), 1]
  )
  ...
  if (cloud == "real") {
    df_ges <- df_ges %>% filter(variable == "real")
  }

  if (cloud == "fitted") {
    df_ges <- df_ges %>% filter(variable == "fitted")
  }
