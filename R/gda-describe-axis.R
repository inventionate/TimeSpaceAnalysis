...
gda_describe_axis <- function(res_gda, axis = 1, contrib = "auto") {
  ...
  ctr_axis <-
    ctr %>%
    select(rowname, ctr = matches(paste0("^Dim.", axis, "$"))) %>%
    arrange(desc(ctr))
  ...
  coord_axis <-
    ...
    data.frame() %>%
    tibble::rownames_to_column() %>%
    filter(rowname %in% ctr_axis$rowname) %>%
    select(rowname, coord = matches(paste0("^Dim.", axis, "$")))

  # Datensatz Kategorien ctr
  df_category <-
    left_join(ctr_axis, coord_axis, by = "rowname") %>%
  ...
  df_dims <-
    df_category %>%
    mutate(coord = ifelse(coord < 0, "negative", "positive")) %>%
    group_by(coord) %>%
    summarise_at(vars(matches("ctr")), sum)

  # Datensatz angezeigte Kategorien gesamt ctr
  df_total <-
    df_dims %>%
    ungroup() %>%
    summarise(ctr = sum(ctr))
